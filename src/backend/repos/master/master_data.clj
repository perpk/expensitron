...
(defn update-masterdata-by-id [id body]
  (jdbc/update! (persistence/db-connection) :master_data_item (json/read-str body :key-fn keyword) (sql/where {:id (Integer/valueOf id)})))

(defn delete-masterdata-by-id [id]
  (jdbc/delete! (persistence/db-connection) :master_data_item (sql/where {:id (Integer/valueOf id)})))

(defn create-masterdata-record [body]
  (jdbc/insert! (persistence/db-connection) :master_data_item (json/read-str body :key-fn keyword)))
